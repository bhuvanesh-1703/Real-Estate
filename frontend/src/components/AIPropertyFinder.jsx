...
export default function AIPropertyFinder({ onSelectProperty }) {
  ...
  const handleAISearch = async (e) => {
    ...
    const budget =
      lower.includes("50") || lower.includes("50l")
        ? "₹50 Lakhs"
        : lower.includes("75")
          ? "₹75 Lakhs"
          : "₹1.5 Crore";
    const propertyType = lower.includes("villa")
      ? "Villa"
    ...
    const bhk = lower.includes("2bhk")
      ...
      : lower.includes("3bhk")
        ...
        : lower.includes("4bhk")
          ? "4 BHK"
          : "3+ BHK";
    const location = lower.includes("madurai")
      ? "Madurai Prime Areas"
      : "Anna Nagar Corridor";
    const purpose =
      lower.includes("investment") || lower.includes("rental")
    ...
    setExtractedData({
      ...
      location,
      purpose,
      preferences: apiRes?.matchReasons
        ? apiRes.matchReasons.join(" • ")
        : "Gated Security, Smart Home Connectivity",
    });

    let matched = PROPERTIES;
    if (propertyType !== "All Luxury Types") {
      matched = PROPERTIES.filter(
        (p) => p.type === propertyType || p.type.includes(propertyType),
      );
    }
  ...
  return (
    <section
      id="ai-finder"
      className="py-20 relative bg-[#0F172A] blueprint-grid text-[#F8FAFC]"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center space-y-4 max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 bg-sky-500/10 px-3.5 py-1.5 rounded-full border border-sky-500/30 text-xs font-mono font-semibold text-sky-400">
            <Sparkles className="w-3.5 h-3.5" />
            INTELLIGENT MATCHMAKING ENGINE
          </div>
          <h2 className="font-serif-fraunces text-3xl sm:text-5xl font-extrabold text-white tracking-tight">
            Tell Us What You’re Looking For.
          </h2>
          <p className="text-sm text-slate-400 font-light">
            Type your requirements in conversational plain language. Our neural
            algorithm analyzes pricing models, floor plans, and locational yield
            to tailor choices.
          </p>
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-12">
          <form
            ...
            className="glass-panel p-3 rounded-2xl border border-slate-700/80 flex flex-col sm:flex-row gap-3 bg-[#1E293B]/80"
          >
            <input
              type="text"
              value={prompt}
              ...
              className="flex-1 bg-transparent px-4 py-3 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none font-mono"
            />
            <button
              type="submit"
              disabled={isSearching}
              className="bg-gradient-to-r from-blue-600 via-sky-500 to-indigo-600 hover:from-blue-500 hover:to-sky-400 text-white font-bold text-xs sm:text-sm px-6 py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 shrink-0 shadow-lg shadow-blue-500/25"
            >
              {isSearching ? (
                <>
          ...
          <div className="flex flex-wrap gap-2 mt-3 text-[11px] justify-center text-gray-400 font-mono">
            ...
            <button
              onClick={() => {
                setPrompt(
                  "Show villas near Madurai with good road connectivity.",
                );
              }}
              className="hover:text-[#B08D57] transition-colors"
            >
            ...
            <button
              onClick={() => {
                setPrompt(
                  "I want an investment property with high rental potential.",
                );
              }}
              className="hover:text-[#B08D57] transition-colors"
            >
          ...
          <div className="glass-panel p-6 rounded-2xl border border-[#B08D57]/40 mb-12 max-w-4xl mx-auto animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-xs font-mono">
              <div className="bg-[#0D1410]/60 p-3 rounded-xl">
                ...
                <span className="font-semibold text-[#EFEAE1]">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="font-serif-fraunces text-lg font-bold text-[#EFEAE1] flex items-center gap-2">
              ...
              <span>
                {recommendations.length} properties match your requirements
              </span>
            </h3>
          </div>
            ...
            {recommendations.map((prop) => (
              ...
              >
                <div>
                  ...
                  <span className="text-[10px] text-gray-400 uppercase tracking-wider font-mono font-semibold">
                    {prop.type}
                  </span>
                  <h4 className="font-serif-fraunces font-bold text-base text-[#EFEAE1] group-hover:text-[#B08D57] transition-colors">
                    {prop.title}
                  </h4>
                  ...
                  <div className="mt-3 p-3 rounded-xl bg-[#0D1410]/60 border border-white/5 text-[11px] space-y-1">
                    <span className="font-semibold text-[#B08D57] block font-mono">
                      Why this property?
                    </span>
                    <p className="text-gray-300 leading-tight font-light">
                      {prop.matchReason}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between">
                  <span className="font-serif-fraunces text-lg font-bold text-[#EFEAE1]">
                    {prop.price}
                  </span>
                  <button
