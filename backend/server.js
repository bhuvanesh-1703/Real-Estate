...
if (isProduction) {
  ...
  if (missingSecrets.length > 0) {
    console.error(`[Auth Fatal] Missing required authentication environment variables in production: ${missingSecrets.join(', ')}`);
    console.error('[Auth Fatal] Startup aborted to prevent unauthorized access via default credentials.');
    process.exit(1);
  }
}
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (allowedOrigins.includes(origin) || allowedOrigins.includes('*') || !isProduction) {
      return callback(null, true);
    }
    return callback(new Error(`CORS policy error: Origin ${origin} is not allowed by FRONTEND_URL configuration.`));
  },
  credentials: true
}));
...
async function startServer() {
  ...
  if (!isDbConnected && isProduction) {
    console.error('[Startup Fatal] Production server startup failed because MongoDB connection could not be established.');
    process.exit(1);
  }

  app.listen(PORT, () => {
    console.log(`[Aetheria Backend] Server running on http://localhost:${PORT}`);
    console.log(`[Aetheria Backend] Environment: ${process.env.NODE_ENV || 'development'}`);
    console.log(`[Aetheria Backend] Database Status: ${isDbConnected ? 'MongoDB Connected' : 'In-Memory Fallback Active'}`);
  });
}
